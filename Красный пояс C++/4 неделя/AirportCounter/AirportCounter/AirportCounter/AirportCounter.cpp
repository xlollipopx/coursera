...
public:
	...
	AirportCounter(TIterator begin, TIterator end) {
		...
		for (auto it = begin; it != end; it++) {
			data_[static_cast<int> (*it)]++;
		}
	}

	// получить количество элементов, равных данному
	size_t Get(TAirport airport) const {
		return data_[static_cast<int>(airport)];
	}

	// добавить данный элемент
	void Insert(TAirport airport) {
		data_[static_cast<int>( airport)]++;
	}
	...
	void EraseAll(TAirport airport) {
		data_[static_cast<int>( airport)] = 0;
	}
	...
	Items GetItems() const {
		...
		for (size_t i = 0; i < data_.size();i++) {
			result[i] = make_pair(static_cast<TAirport>(i), data_[i]);
		}
		return result;
	}

private:
	array<int, static_cast<size_t>(TAirport::Last_)> data_;
};
...
void TestManyGetItems() {
	...
	uint64_t total = 0;
	for (int step = 0; step < 100'00; ++step) {
