...
private:
	...
	Iterator last;
public:
	Page(Iterator _begin, Iterator _end) : first(_begin), last(_end) {}
...
public:
	Paginator(Iterator _begin, Iterator _end, size_t _page_size) : Begin(_begin), End(_end), page_size(_page_size) {
		auto begin_t = _begin;
		size_t same = (End - Begin) / _page_size;
		for (int i = 0; i < same; i++) {
			Page<Iterator> p(begin_t, begin_t + page_size);
			pages.push_back(p);

			begin_t += (page_size);
		}
		if ((End - Begin) % _page_size != 0) {
			Page<Iterator> p(begin_t, End);
			pages.push_back(p);
		}
	}
